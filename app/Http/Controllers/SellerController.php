...
class SellerController extends Controller
{
    ...
    public function storeProductRequests(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                $orderRequest->requestNO = uniqid(); // Generate a unique request number
    ...
    private function assignAgent($countryFrom, $countryTo){
        // Fetch all agents (role 2) with their countries
        $agents = User::where('role', 2)
            ->with(['sourcingCountries', 'destinationCountries']) // Eager load relationships
            ->get();
    
        // Filter agents based on the sourcing and destination countries
        ...
        // Logic to select an agent
        if ($availableAgents->count() === 1) {
            // If exactly one agent is available, return their ID
            return $availableAgents->first()->id;
        } elseif ($availableAgents->count() > 1) {
            // If more than one agent is available, use a dispatching mechanism
            return $this->dispatchAgent($availableAgents);
        } else {
            // If no agents are available, return null or handle this case as needed
            return null;
        }
    }
